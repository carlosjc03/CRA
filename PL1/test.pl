...
test2_monopolio :-
    ...
    turno_limpio(EstadoPrueba, EstadoFinal, simulacion),
    EstadoFinal = estado([Jugador1Final | _], _, _, _),
    (verificar_monopolio(Jugador1Final, marron) -> write('[EXITO]: Monopolio detectado.'), nl ; write('[ERROR]'), nl).
...
test3_bancarrota :-
    ...
    turno_limpio(EstadoPrueba, _, simulacion).
...
test4_alquileres :-
    ...
    turno_limpio(EstadoPrueba, EstadoMitad, simulacion),
    turno_limpio(EstadoMitad, _, simulacion).

test5_simulacion :-
    estado_inicial(E0),
    write('>> ESCENARIO 5: Simulacion 10 turnos'), nl,
    bucle_juego(E0, 10).


test_todos :-
    test1_compras, nl, test2_monopolio, nl, test3_bancarrota, nl, test4_alquileres, nl, test5_simulacion, nl,
